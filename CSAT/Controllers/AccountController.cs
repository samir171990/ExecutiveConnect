    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult UserProfile(ProfileViewModel model)
        {
            ...
            if (user != null)
            {
                ...
                {
                    string fileName = "";
                    #region Profile File Processing
                ...
                System.IO.Path.GetExtension(file.FileName);
                            var uploadUrl = Server.MapPath("~/ProfileImages");
                            file.SaveAs(Path.Combine(uploadUrl, fileName));
                        }
                    }
                    #endregion

                    var getUser = db.UserAdditionalDatas.Where(p => p.aspnetUserId == user).FirstOrDefault();
                    if (getUser != null)
                    {
                        DateTime dateOfBirth = new DateTime(model.Year, model.Month, model.Day);
                        ...
                        getUser.ProfileImage = !string.IsNullOrEmpty(fileName) ? fileName : getUser.ProfileImage;
                        getUser.ExecutiveTitle = model.ExecutiveTitle;
                        getUser.UserBio = model.UserBio;
                        db.SaveChanges();
